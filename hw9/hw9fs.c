...
/* all the structs below have *many* more members, 
which we don't need to initialize for this homework */
...
// you'll probably want to change this struct
struct disk_header {
	char dummy[1024];
};
...
hw9fs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	((struct disk_header*)sb->s_fs_info)->dummy[100]=0; // add a silly null-terminator just so we can print something short to the log
